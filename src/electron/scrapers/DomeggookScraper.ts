...
import type { ExtractedBasicInfo, ImageCollectResult, Scraper } from './BaseScraper'

export class DomeggookScraper implements Scraper {
  ...
  async collectImages(page: Page, vendor: VendorConfig, productDir?: string): Promise<ImageCollectResult> {
    ...
    if (vendor.detail_image_xpath) {
      ...
      // 상세 영역 스크린샷은 호출 측에서 제공하는 헬퍼가 더 좋지만, 여기서는 페이지 전체 스크롤 캡쳐 최소 구현 생략
      // 호출부에서 대체되므로 여기서는 null 유지
      detailCapturePath = null
    }

    // 메인 이미지 URL만 반환하고 저장은 상위 헬퍼에서 처리하도록 단순화할 수도 있으나, 여기서는 상위 규칙 준수
    // 실제 저장은 상위 자동화 클래스의 _downloadToBuffer/_saveJpg 사용이 필요하므로 여기서 경로 저장은 생략

    return { savedMainImages: mainImageUrls.slice(0, 4), detailCapturePath }
  }
